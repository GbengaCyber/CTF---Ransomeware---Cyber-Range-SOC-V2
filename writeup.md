...
The following artefacts were discovered during the investigation, establishing the initial scope before the technical analysis began.

### Ransom Note — `akira_readme`

[![Akira Ransom Note](./ransom_note.png)](./ransom_note.png)

The `akira_readme` text file was dropped across all encrypted directories. Key details extracted:
- **Encryption:** AES-256, `.akira` extension appended to all files
- **Exfiltration claimed:** Financial docs, employee PII, client databases, contracts, internal comms
- **TOR contact:** `akiral2iz6a7qgd3ayp3l6yub7xx2uep76idk3u2kollpj5z3z636bad.onion`
- **Victim ID:** `813R-QWJM-XKIJ`
- **Deadline:** 72 hours before data publication

---

### Encrypted File Server — `C:\Shares\`

[![Encrypted Files](./encrypted_files.png)](./encrypted_files.png)

The file server share at `C:\Shares\` was fully encrypted. All business-critical folders were hit — Backups, Clients, Compliance, Contractors, and Payroll. Files within subdirectories (e.g. `Clients\Barclays_PLC`) show the `.akira` extension with a modification timestamp of `1/27/2026 10:18 PM`.

---

### Ransom Negotiation — Akira Secure Chat

[![Akira Secure Chat](./akira_chat.png)](./akira_chat.png)

The victim engaged with the threat actor via the TOR-hosted Akira Secure Chat portal. The attacker opened at **£65,000**, the victim countered with **£11,000**. The attacker responded with a 48-hour ultimatum. This chat confirms active double-extortion and a live negotiation session.
...
AnyDesk relay connections were reviewed across all compromised hosts. `relay-0b975d23.net.anydesk.com` was the last relay connection observed on `as-srv` — active just 10 minutes before ransomware deployment, confirming the attacker maintained remote access right up to encryption.
...
`DeviceFileEvents` was queried for `.bat` files on compromised hosts. `kill.bat` was identified as the security evasion script, downloaded from `sync.cloud-endpoint.net/kill.bat` via `bitsadmin` and executed to disable Windows Defender and delete shadow copies before ransomware deployment.
...
Windows Defender was disabled via a registry modification setting `DisableAntiSpyware = 1` under `HKLM\SOFTWARE\Policies\Microsoft\Windows Defender`. Part of the defence evasion sequence executed before credential theft and ransomware deployment.
...
`DeviceRegistryEvents` filtered for Defender-related registry modifications. `DisableAntiSpyware` was set to `1` via `reg add` at `21:03:42` — approximately 75 minutes before encryption.
...
AnyDesk relay connections identified across `as-srv`, `as-pc1`, and `as-pc2` throughout the attack window. Pre-staged during the *The Broker* compromise and reused 12 days later to re-enter the environment without re-exploitation.
...
`DeviceFileEvents` queried for `AnyDesk.exe` across the full investigation window. The binary was dropped into `C:\Users\Public\` — a world-writable directory — rather than the legitimate install path `C:\Program Files (x86)\AnyDesk\`, confirming manual staging by the attacker.
...
`DeviceFileEvents` queried for executables dropped into staging directories. `wsync.exe` was identified as the new C2 beacon, dropped into `C:\ProgramData\` by `powershell.exe`. Masquerades as a Windows sync utility — responsible for all C2 comms, payload delivery, and spawning attacker commands.
...
Confirmed from the Q19 query. `wsync.exe` consistently written to `C:\ProgramData\wsync.exe` across all three drop events. Common attacker staging path — accessible without user-specific permissions.
...
Three drops of `wsync.exe` observed. The first at `20:22` carried a different SHA256 from the final version — the original beacon that failed to maintain stable C2 communications.
...
`DeviceFileEvents` filtered for executables in user directories. `scan.exe` downloaded directly to `C:\Users\david.mitchell\Downloads\` — an NSIS self-extracting installer that unpacked Advanced IP Scanner for network reconnaissance.
...
`scan.exe` showed no arguments directly. Child process analysis revealed `scan.exe` → `scan.tmp` (NSIS extractor) → `advanced_ip_scanner.exe` with `/portable` flag — avoiding installation to leave minimal forensic trace.
...
Compromised admin account `as.srv.administrator` used for all attacker activity on `as-srv`. First seen in logon events from `as-pc2` (`10.1.0.183`) on `1/15/2026` during *The Broker* compromise and reused throughout the Akira deployment.
...
`DeviceProcessEvents` filtered for LOLBIN download commands. `bitsadmin.exe` used to pull tools from `sync.cloud-endpoint.net` but failed multiple times — first with a doubled destination path, then a non-existent `C:\Temp\` — before succeeding on the fourth attempt to `Downloads\`.
...
After `bitsadmin` issues, the attacker switched to PowerShell's `Invoke-WebRequest`. Used across `as-pc2` and `as-srv` to download `wsync.exe`, `updater.exe`, `st.exe`, and `clean.bat`. URL obfuscation was also observed — domain splitting, base64 encoding, and variable splitting to evade detection.
...
`DeviceFileEvents` queried for archive creation events. `st.exe` created `exfil_data.zip` at `C:\Users\Public\` on `as-srv` — 6 minutes after encryption. Staged for exfiltration.
...
`st.exe` created `exfil_data.zip` in `C:\Users\Public\` — same staging directory used for AnyDesk. Archive exfiltrated to `sync.cloud-endpoint.net` via `Invoke-WebRequest` POST at `22:24:55`.
...
`DeviceProcessEvents` reviewed around the encryption timestamp. `updater.exe` executed from `C:\ProgramData\` by `powershell.exe` at `22:18:29`. Named to blend with the legitimate Google Updater process on the same host. Immediately spawned `wevtutil.exe` to clear five Windows event logs.
...
`DeviceFileEvents` filtered for `.txt` creation. `updater.exe` dropped `akira_readme.txt` four times across encrypted directories at `22:18:33` — 4 seconds after execution — confirming the ransomware handles both encryption and ransom note delivery.
...
Identified from the Q37 query. `akira_readme.txt` first created at `22:18:33 UTC`, 4 seconds after `updater.exe` execution at `22:18:29`.
...
`DeviceProcessEvents` reviewed post-encryption. `powershell.exe` executed `cmd.exe /c C:\ProgramData\clean.bat` at `22:20:27` — 2 minutes after ransomware execution. Downloaded from `sync.cloud-endpoint.net/kill.bat` earlier in the attack, responsible for removing `updater.exe` and other artefacts.
